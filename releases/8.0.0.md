...
- Add query interface, REST serializers for retrieving & updating tile data by node alias [#11595](https://github.com/archesproject/arches/issues/11595)
...
1. In settings.py:
    - add the following key to `DATABASES` to [improve indexing performance](https://github.com/archesproject/arches/issues/11382):
        ```
            "OPTIONS": {
                "options": "-c cursor_tuple_fraction=1",
            },
        ```
    - add `rest_framework` to `INSTALLED_APPS` if you wish to use an [ArchesResourceSerializer]() to build REST APIs for your resource models.
